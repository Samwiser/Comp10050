...
int main(void)
{
	...
	while(num_players<1 || num_players>6)			//User inputs number of players
	{
		printf("enter number of players(max 6): ");
		scanf("%d", &num_players);
	}
	
	for(i=0; i<num_players; i++)				//User inputs the player name and type
	{
	...
	while(num_slots<num_players || num_slots>20)	//User inputs number of slots
	{
		printf("\nenter number of slots(max 20): ");
		scanf("%d", &num_slots);
	}
	
	player_abilities(num_players);			//Function to give player abilities is called
	
	printf("\n");
	
	slot_sorting(num_slots, rand_num, num_players);	//Function to create and sort players in slots is called
	
	printf("Move player/Attack Player \n\n");	//User chooses if they want ot attack or move their player
	printf("1 - Move player \n");
	printf("2 - Attack nearest Player \n\n");
	
	for( k = 0 ; k < num_players ; k++ )
	{
		printf("Player: %s \n", players_array[k].player_name);		//Each player is asked what they want
		printf("Please enter what you would like to do with your %s:", players_array[k].type);
		
		scanf("%d", &n);
		
		if( n == 1 )			//Functions called based on what player wants to do
		{
	...
	for( i = 0; i < num_players; i++ )	//Player data printed at the end
	{
...
void slot_sorting(int num_slots, int rand_num, int num_players)
{
	...
	for(i=0; i<num_slots; i++)		//Algorithim to Create slots based on number generated by rand gunction
	{
		rand_num = ((rand() %3)+1);
	...
	{
		j = (rand() % num_slots);		//Each player is placed into random slot
		
		strcpy(array2[j].player_inf, players_array[i].player_name);
		strcpy(array2[j].type_inf, players_array[i].type);	
		players_array[i].position = j;		//Save information on player struct
	}

	for(i=0; i<num_slots; i++)			//Print out player location and slots
	{
...
void player_abilities(int num_players)
{
	...
	for(i=0; i<num_players; i++)				//Algorithim to give player type stats based on the type of player they are
	{
...
void move_player(int num_slots, int num_players, int k)
{
	...
	if(strcmp(array2[(players_array[k].position) + 1].player_inf,"empty")!=0 && strcmp(array2[(players_array[k].position) - 1].player_inf,"empty")!=0) //Checks to see if player can move if not then prints it
	{
		printf("You cannot move to any position \n\n");
	}
	else if(strcmp(array2[(players_array[k].position) + 1].player_inf,"empty")!=0)		//Algorithim moves player back and changes stats based on new slots
	{
	...
	else if(strcmp(array2[(players_array[k].position) - 1].player_inf,"empty")!=0 )		//Algorithim moves player forward and changes stats based on new slots
	{
	...
	{
		printf("You are able to move backwards or forwards\n");		//If player can move either direction then player chooses and algorithim moves the player
...
void attack_player(int num_slots, int num_players, int k)
{
	...
	while(b < num_slots)		//Algorithim to find nearest player
	{
	...
	if((b - k) > (k - c))		//Prints out nearest player and check stats to see if player loses life points or enemy does
	{
